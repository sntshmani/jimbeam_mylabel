...
int main(int argc, char *argv[])
{
	try 
	{
		...
		try 
		{
			...
			OCRModule::ImageToPDF(doc.GetSDFDoc(), input_path + "psychomachia_excerpt.png", NULL);
		...
		try
		{
			...
			OCRModule::ImageToPDF(doc.GetSDFDoc(), input_path + "multi_lang.jpg", &opts);
		...
		try
		{
			...
			SDFDoc doc((input_path + "german_kids_song.pdf").c_str());
			...
			ignore_zones.AddRect(1768, 680, 2056, 3044);
		...
		try
		{
			...
			SDFDoc doc((input_path + "zero_value_test_no_text.pdf").c_str());
